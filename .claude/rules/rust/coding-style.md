...
> This file extends [coding-style.md](../coding-style.md) with Rust-specific content.
...
- **Libraries**: define typed errors; use `thiserror` only if the dependency is already present or justified
- **Applications**: use flexible error context with the existing project error strategy
...
- Only mark `pub` what belongs on the crate or FFI-facing API
- Re-export crate-facing API from `lib.rs` when it clarifies boundaries
