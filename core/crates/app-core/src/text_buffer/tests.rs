//! Unit tests for the editable text buffer. A child module (not an external
//! test file) so the differential oracle and structural assertions keep their
//! access to the buffer's private internals (`root`, `from_source_with_cap`,
//! the rope's `pub(super)` introspection helpers).
