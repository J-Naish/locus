...
struct TextDocumentEditorView: NSViewRepresentable {
    ...
    final class Coordinator: NSObject, NSTextViewDelegate, @preconcurrency NSTextStorageDelegate {
        ...
        func configure(
            ...
            textView.selectedRanges = selectedRanges.clamped(toTextLength: (textView.string as NSString).length)
        ...
        func textStorage(
            ...
            guard !isUpdatingTextView,
                  !isApplyingHighlight,
                  editedMask.contains(.editedCharacters) else {
                return
            }
...
private extension [NSValue] {
    func clamped(toTextLength textLength: Int) -> [NSValue] {
        map { value in
            ...
            return NSValue(range: NSRange(location: range.location, length: Swift.min(range.length, maximumLength)))
        }
    }
}
