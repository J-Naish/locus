...
- The terminal pane renders exclusively via Metal
  (`TerminalMetalRenderer`); correctness is pinned by readback unit tests
  over the shared shaping pipeline. There is no CG fallback — a Metal-less
  environment logs once and renders nothing.
