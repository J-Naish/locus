...
Locus lets users open local folders and will later remember recent folders. The macOS app is not sandboxed yet, but distribution will require a clear sandbox and file-access model.

SwiftUI `fileImporter` returns user-selected URLs that can be accessed immediately with `startAccessingSecurityScopedResource()`. That is enough for the current folder-list flow, but it is not enough for persistent recents and later editor sessions after relaunch.
...
- Persistent folder access stores security-scoped bookmark data when recents are implemented.
